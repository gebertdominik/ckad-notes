...
* According to k8s website it's an "open-source system for automating deployment, scaling, and management of contenerized applications".
...
* a need for an infrastucture to run containers and watch over them when things fail and self-heal.
...
For testing puposes it can be combined to a single node.
...
K8s master runs various server and manager processes for the cluster. Among the componens of the master node are:
...
* checks avaliable resources(CPU etc.) to bind, and assign the Pod.
